          ...
          <TransitionChild enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
            <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
              <div class="sm:flex sm:items-start">
                <div v-if="danger" class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10">
                  <ExclamationTriangleIcon class="h-6 w-6 text-red-600" />
                </div>
                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                  <DialogTitle as="h3" class="text-base font-semibold leading-6 text-gray-900">{{ title || $t('common.confirm') }}</DialogTitle>
                  <div class="mt-2">
              ...
              <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                <button @click="emit('confirm')" :class="[danger ? 'btn-danger' : 'btn-primary', 'w-full sm:ml-3 sm:w-auto sm:text-sm']">
                  {{ confirmLabel || $t('common.confirm') }}
                </button>
                <button @click="emit('close')" class="btn-secondary mt-3 w-full sm:mt-0 sm:w-auto sm:text-sm">
                  {{ $t('common.cancel') }}
                </button>
              </div>
            </DialogPanel>
